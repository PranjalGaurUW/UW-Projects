...
head(dat)
summary(dat$HR_10)
m <- polr(as.factor(HR_10) ~ HR_05C + RURURB + AGEGRP + SEX, data = dat, Hess=TRUE)
summary(m)
# write.table(summary(m), "H1.csv", sep=",")
...
(s <- with(dat, summary(as.numeric(HR_10) ~ HR_05C + RURURB + AGEGRP + SEX, fun=sf)))

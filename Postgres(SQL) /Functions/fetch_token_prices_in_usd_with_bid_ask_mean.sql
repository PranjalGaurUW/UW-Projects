...
begin
	...
	select bid, ask from cex_prices cp 
				...
				and cp.datetime::timestamp with time zone <= txn_date order by cp.datetime desc limit 1 into bid_price, ask_price;
	
	--Taking bid/ask mean
	select (bid_price + ask_price)/2 into currency_price_usd;
	...
	VALUES (currency_price_usd*New.fee, New.id);
	 ...
	 THEN
				...
				--Taking bid/ask mean
				select (bid_price + ask_price)/2 into value_usd;
